...
DirectX::XMMATRIX Object::TransformToMatrix()
{
	const DirectX::XMVECTOR scale = DirectX::XMLoadFloat3(&localTransform.scale);
	const DirectX::XMVECTOR rotation = DirectX::XMLoadFloat3(&localTransform.rotation);
	const DirectX::XMVECTOR position = DirectX::XMLoadFloat3(&localTransform.position);

	const DirectX::XMMATRIX S = DirectX::XMMatrixScalingFromVector(scale);
	const DirectX::XMMATRIX R = DirectX::XMMatrixRotationRollPitchYawFromVector(rotation);
	const DirectX::XMMATRIX T = DirectX::XMMatrixTranslationFromVector(position);

	return S * R * T;
}

void Object::UpdateWorldMatrix(const DirectX::XMMATRIX& parentWorldMatrix)
{
	DirectX::XMMATRIX localMatrix = TransformToMatrix();
